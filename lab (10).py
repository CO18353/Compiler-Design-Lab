...
def main():
    ...
    for i, j in table.items():
        ...
        print("|")
        #print('\t|  '.join(list(j.get(sym,' ') if type(j.get(sym))in (str , None) else next(iter(j.get(sym,' ')))  for sym in sym_list)),'\t\t|')
